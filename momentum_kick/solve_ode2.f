...
!          write(*,*) "enter fex subroutine"
          allocate(ham(neq,neq))
!          write(*,*) "ham are allocated"
!          call form_matrix_2d(neq, Ham, T)
!          call form_matrix_1d(neq,Ham,T)

          !call wavepacket_ham(neq, Ham, T)
          
!          if (T < 1.D-8) then
!            call wavepacket_ham(neq, Ham, T)
           !write(*,*) "wavepacket_ham has been called"
!          else
            call form_matrix_1d(neq, Ham, T)
!            write(*,*) "form_matrix_1d has been called"
!          endif

           call gemv(Ham, Y, YDOT)
           YDOT = -RPAR*YDOT
