...
#figure A
gsA=gridspec.GridSpec(2,2,width_ratios=[5,1],height_ratios=[1,10])
left = xmin
right = xmin + width
bottom = ymin + height + left_hspace
top = bottom + height
gsA.update(wspace=subplot_wspace,hspace=subplot_hspace,left=left,right=right, bottom=bottom, top=top )

axA = plt.subplot(gsA[1,0:2])
...
axA_colorbar1 = plt.subplot(gsA[0,0])
axA_colorbar1.tick_params(direction='out',bottom='off', top='on', left='on', right='off')
...
cb1.ax.set_xticklabels(['0','0.007',\
                     '0.014'],fontsize=13,fontproperties=tnr)
                     

axA_colorbar2 = plt.subplot(gsA[0,1])
axA_colorbar2.tick_params(direction='out',bottom='off', top='on', left='on', right='off')
...
#move labels to the top
cb1.ax.xaxis.tick_top()
cb1.ax.set_xticklabels(['','0.138'],fontsize=13,fontproperties=tnr)


# figure C
gsC=gridspec.GridSpec(2,2,width_ratios=[5,1],height_ratios=[1,10])
left = xmin
right = xmin + width
bottom = ymin 
top = bottom + height
gsC.update(wspace=subplot_wspace,hspace=subplot_hspace,left=left,right=right, bottom=bottom, top=top )

axC = plt.subplot(gsC[1,0:2])
plt.tick_params(direction='out',bottom='on', top='off', left='on', right='off')
...
axC_colorbar1 = plt.subplot(gsC[0,0])
axC_colorbar1.tick_params(direction='out',bottom='on', top='off', left='on', right='off')
cb1 = mpl.colorbar.ColorbarBase(axC_colorbar1, ticks=[0,0.5,1.0], cmap=cm.get_cmap('PuRd'), orientation='horizontal')
#move labels to the top
cb1.ax.xaxis.tick_top()
cb1.ax.set_xticklabels(['0','0.005',\
                     '0.011'],fontsize=13,fontproperties=tnr)

axC_colorbar2 = plt.subplot(gsC[0,1])
axC_colorbar2.tick_params(direction='out',bottom='on', top='off', left='on', right='off')
cmap = mpl.colors.ListedColormap([mpl.colors.rgb2hex([0.40392157435417175, 0.0, 0.12156862765550613]), \
                                  mpl.colors.rgb2hex([0.40392157435417175, 0.0, 0.12156862765550613])])
cb1 = mpl.colorbar.ColorbarBase(axC_colorbar2, cmap=cmap, ticks=[0,1.0], orientation='horizontal')
#move labels to the top
cb1.ax.xaxis.tick_top()
cb1.ax.set_xticklabels(['','0.035'],fontsize=13,fontproperties=tnr)



# figure B and D
gsBD=gridspec.GridSpec(2,1, height_ratios=[1,1])
left = xmin + width + LR_wspace 
right = left + width
bottom = ymin
top = bottom + 2*height + left_hspace
gsBD.update(hspace=0.4,left=left,right=right, bottom=bottom, top=top )

axB = plt.subplot(gsBD[0,0])
