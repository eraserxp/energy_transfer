...
Cf2py intent(in) ka
        ...
        implicit none
      	DOUBLE PRECISION, PARAMETER :: Pi=3.141592653589793115997963468d0
        double precision, parameter :: variance = 0.005D0
...
Cf2py intent(in) kay
        ...
        implicit none
      	DOUBLE PRECISION, PARAMETER :: Pi=3.141592653589793115997963468d0
        double precision, parameter :: variance = 0.005D0 !0.005D0
        double precision, parameter :: central_kax = pi/2
        double precision, parameter :: central_kay = pi/2 
        double precision :: kax, kay
...
!H1 is independent of angle
!H2 is dependent on angle
Cf2py intent(in) neq
Cf2py intent(out) Ham
Cf2py intent(in) T
        use molecule_information
        implicit none
      	DOUBLE PRECISION, PARAMETER :: Pi=3.141592653589793115997963468d0
        ...
        double precision :: T
        double precision, allocatable, save :: H1(:,:), H_sin(:,:), H_cos(:,:)
...
!             make them symmetric 
        ...
        Ham = (1 - 3*(dcos(theta)**2)*(dcos(phi)*H_cos + dsin(phi)*H_sin)**2 )*H1
        return
...
!--------------------------------------------------------------------------------------------
          case(7)
            if ((time >= 0.D0) .AND. (time < 1.D-3) ) then
              theta_vs_time = 0.D0
            else if ((time >= 1.D-3).and.(time <= 2.D-3)) then
              theta_vs_time =0.5*pi*( DSIN(pi*(time - 1.D-3)/(2*1.D-3)) )**2  
            else
              theta_vs_time = pi/2
            endif        
